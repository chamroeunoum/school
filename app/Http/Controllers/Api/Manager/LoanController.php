    ...
    public function index(Request $request){ 
        ...
        if( $user == null ){
            return response()->json([
                'message' => "សូមចូលប្រើប្រាស់ម្ដងទៀត។"
            ],403);
        }
        ...
        // Get clients of each store which own by the owner
        $peopleIds = [] ;
        foreach( $user->stores AS $storeIndex => $store ){
            $storeClientQuery = $store->users()->whereHas('roles',function($query){
                $query->where('tag','webapp');
            });
            if( $storeClientQuery->count() ){
                $peopleIds = array_merge( $peopleIds , $storeClientQuery->get()->map(function($client){ return $client->id ;})->toArray() );
            }
        }
